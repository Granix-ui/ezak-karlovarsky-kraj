...
st.markdown("Prohledávání všech zadavatelů v kraji (75 profilů)")

base_index = "https://ezak.kr-karlovarsky.cz/profile_index.html"

# Načtení všech zadavatelů při spuštění
...
# Zobrazení tabulky se všemi zadavateli
...
if st.button("🔄 Načíst aktivní zakázky ze všech zadavatelů", type="primary"):
    ...
    for idx, row in df_profiles.iterrows():
        ...
        try:
            # Přidáme filtr aktivních zakázek
            url = profile_url
            if "?" not in url:
    ...
    if total_active == 0:
        st.info("Momentálně nejsou žádné aktivní zakázky u žádného zadavatele v Karlovarském kraji.")
    else:
